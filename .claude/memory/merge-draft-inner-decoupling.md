...
1. **inner 是两个参数，不是一个**。extract 时刻它生成 clip_mask.png（黑=merge 时被 AI 内容替换的内圈、白=保留原画面的锚定环；注意掩膜不提供给外部 AI 工具，见 [[inner-is-merge-only]]），随版本冻结；merge 时刻它是「贴哪、融哪」的混合窗口（拉普拉斯+羽化的取值域），是创作决策。解耦只许解后者。当时考虑过「inner 不再写入 clip.json」——**错**：字段集是对外契约（硬性规则 2、test_case 冻结），删字段还毁掉混合区域的版本记录和 reset-draft 的恢复源。正确姿势：**写侧一字不动，只改读侧**（merge 显式传 --inner；CLI 不传时仍回落版本值，CLI 用户与像素基线双向兼容）。
